...
# 导入的另一个py文件中的方法
from onedrive import set_onedrive
from mysql import get
from mysql import set
...
async def worker(name):
    ...
    while True:
        ...
        print(f"{name}  {get_local_time()} 开始下载： {chat_title} - {file_name}")
        ...
        try:
            ...
            if upload_file_set:
                # print("上传")
                # 获取文件类型 mp4 png jpg
                file_type = str(os.path.splitext(file_save_path)[-1]).split('.')[1]

                # 执行上传 参数1：文件名称 参数2：频道名称 参数3：文件类型
                # upload_datas = loop.create_task(set_onedrive('好看视频--轻松有收获.mp4', chat_title, file_type))
                upload_datas = loop.create_task(set_onedrive(file_save_path, chat_title, file_type))
                await asyncio.wait_for(upload_datas, timeout=3300)
                upload_data = upload_datas.result()
                # print(upload_data)
                if upload_data != '':
                    # 上传成功
                    ...
                    # 插入数据库
                    pd = set(file_type, upload_data['url'], upload_data['file_name'], upload_data['file_path'], chat_title, chat_id,file_id)
...
# 接受到/sl  查询队列数量
@events.register(events.NewMessage(pattern='/sl1', from_users=admin_id))
async def get_size(update):
    await bot.send_message(admin_id, f'任务队列数量：{queue.qsize()}')


# 接受到/start
@events.register(events.NewMessage(pattern='/start1', from_users=admin_id))
async def handler(update):
    ...
    if chat_title:
        ...
        last_msg_id = 0
        # 获取频道的消息 参数1：频道实体 参数2：从第几个消息开始获取
