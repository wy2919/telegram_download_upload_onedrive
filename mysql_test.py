...
def set(type,file_name,file_path,pd_name,pd_id,file_id):
    connect = pymysql.Connect(
        host='remotemysql.com',  # 服务器地址
        port=3306,  # 端口
        user='xnf9bOerw3',  # 数据库用户名
    ...
    try:
        ...
        sql="insert into data_test(type,file_name,file_path,pd_name,pd_id,file_id) values ('%s','%s','%s','%s','%s','%s')"
        data = (type,file_name,file_path,pd_name,pd_id,file_id)
        ...
        get_Connect(1)
        return set(type,file_name,file_path,pd_name,pd_id,file_id)
...
def get(file_id):
    ...
    try:
        ...
        # 查询数据
        sql = "SELECT file_id FROM data_test WHERE file_id = '{}'".format(file_id)
        cursor.execute(sql)
